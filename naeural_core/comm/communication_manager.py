...
class CommunicationManager(Manager, _ConfigHandlerMixin):
  ...
  def __select_communicators_and_send(
      self,
      data,
      communicator_name,
      local_only=False,
    ...
    if self.__local_communication_enabled and local_communicator is not None and self.__local_communication_active:
      admission['attempted'].append('local')
      if local_communicator.send(data):
        admission['accepted_by'].append('local')
      else:
        admission['rejected_by'].append('local')

    if not local_only:
      admission['attempted'].append('central')
      if central_communicator.send(data):
        admission['accepted_by'].append('central')
      else:
        admission['rejected_by'].append('central')

    admission.update({
      'any_admitted': len(admission['accepted_by']) > 0,
      'fully_admitted': (
        len(admission['attempted']) > 0
        and len(admission['rejected_by']) == 0
      ),
  ...
  def send(self, data, event_type='PAYLOAD'):
    if event_type == 'COMMAND':
      ...
      if self.should_bypass_commands_to_self and is_command_to_self:
        ...
        return self.__select_communicators_and_send(
          (receiver_id, receiver_addr, command),
  ...
  def get_comms_status(self):
    ...
    for name in keys:
      ...
      if comm is not None:
        ...
        if getattr(comm, 'receives_heartbeat_channel', False):
          comm_status['HEARTBEAT_INGRESS'] = comm.get_heartbeat_ingress_status()
        inbound_queue = comm.get_inbound_queue_status()
        if inbound_queue.get('observable'):
          comm_status['COMMAND_INBOUND_QUEUE'] = inbound_queue
        if name in [
          ct.COMMS.COMMUNICATION_COMMAND_AND_CONTROL,
          "L_" + ct.COMMS.COMMUNICATION_COMMAND_AND_CONTROL,
        ]:
          comm_status['COMMAND_OUTBOUND_QUEUE'] = comm.get_outbound_queue_status()
          transport_delivery = comm.get_transport_delivery_status()
          if transport_delivery is not None:
            comm_status['MQTT_DELIVERY'] = transport_delivery
        dct_stats[name] = comm_status
    return dct_stats
