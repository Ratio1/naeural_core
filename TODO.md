...
- [ ] Expand TRT build diagnostics by ~5x: log GPU name, device index, compute capability, CUDA driver/runtime, Torch/TRT versions, ONNX size/md5, builder flags, optimization profile shapes, and ONNX parser errors; enable verbose TRT logging during build so GPU/TRT issues are traceable.
